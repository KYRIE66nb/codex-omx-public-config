...
SOFFICE_TIMEOUT_SEC: int = int(os.environ.get("DOCX_SOFFICE_TIMEOUT_SEC", "45"))
QUICKLOOK_TIMEOUT_SEC: int = int(os.environ.get("DOCX_QUICKLOOK_TIMEOUT_SEC", "30"))


def ensure_system_tools() -> None:
    missing: list[str] = []
    if sys.platform == "darwin":
        if which("soffice") is None and which("qlmanage") is None:
            missing.append("soffice or qlmanage")
    elif which("soffice") is None:
    ...
    if missing:
        tools = ", ".join(missing)
        raise RuntimeError(
            f"Missing required system tool(s): {tools}. Install the missing renderer(s) and Poppler, then retry."
        )
...
def calc_dpi_via_pdf(input_path: str, max_w_px: int, max_h_px: int) -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="soffice_profile_") as user_profile:
        with tempfile.TemporaryDirectory(prefix="soffice_convert_") as convert_tmp_dir:
            ...
            pdf_path, preview_html = convert_to_pdf(
                input_path, user_profile, convert_tmp_dir, stem
            )
            if not (pdf_path and exists(pdf_path)):
                if preview_html:
                    raise RuntimeError(
                        "LibreOffice timed out during PDF conversion; Quick Look preview "
                        f"was exported to {preview_html}."
                    )
...
def export_quicklook_preview(doc_path: str, out_dir: str) -> str:
    if sys.platform != "darwin" or which("qlmanage") is None:
        return ""

    result = run_cmd_no_check(
        [
            "qlmanage",
            "-p",
            "-o",
        ...
        timeout_sec=QUICKLOOK_TIMEOUT_SEC,
    )
    if result is None:
        return ""

    preview_root = join(out_dir, basename(doc_path) + ".qlpreview")
    preview_html = join(preview_root, "Preview.html")
    return preview_html if exists(preview_html) else ""


def soffice_profile_args(user_profile: str) -> list[str]:
    # On this macOS setup, using a temp UserInstallation causes headless convert-to
    # to hang. Keep the default profile there and retain temp profiles elsewhere.
    if sys.platform == "darwin":
        return []
    return ["-env:UserInstallation=file://" + user_profile]
...
def convert_to_pdf(
    ...
    cmd_pdf = [
        "soffice",
        *soffice_profile_args(user_profile),
        "--invisible",
        "--headless",
        "--norestore",
        "--convert-to",
    ...
    result = run_cmd_no_check(cmd_pdf, timeout_sec=SOFFICE_TIMEOUT_SEC)
    if result is None:
        preview_html = export_quicklook_preview(doc_path, convert_tmp_dir)
        return "", preview_html
    ...
    result = run_cmd_no_check(cmd_odt, timeout_sec=SOFFICE_TIMEOUT_SEC)
    if result is None:
        preview_html = export_quicklook_preview(doc_path, convert_tmp_dir)
        return "", preview_html
    ...
    if exists(odt_path):
        ...
        result = run_cmd_no_check(cmd_odt_pdf, timeout_sec=SOFFICE_TIMEOUT_SEC)
        if result is None:
            preview_html = export_quicklook_preview(doc_path, convert_tmp_dir)
            return "", preview_html
        if exists(pdf_path):
...
def main() -> None:
    ...
    try:
        ...
        result_paths = rasterize(input_path, out_dir, dpi)
        if len(result_paths) == 1 and result_paths[0].endswith(".html"):
            print("Quick Look preview exported to " + result_paths[0])
        else:
